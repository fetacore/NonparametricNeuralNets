#'	A two layer neural network for univariate regression (one input for one output) with Cross Entropy Loss.
#'
#'
...
NNtrainCE <- function(y, x, iter=200, hidden=3, eta=0.005, ahidden='tanh', aout='Linear'){
  ...
  while (!isTRUE(j == iter+1)){
    ...
    for (i in 1:T){
      ...
      error[i] <- (1/T) * (target * log(abs(y_output[i])) + (1-target)*log(abs(1-y_output[i])))
    }
